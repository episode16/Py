...
if __name__ == "__main__":
    try:
        ...
        third_method_time = timeit(lambda: third_method(emails), number=90000000)

        if (
            first_method_time <= second_method_time
            and first_method_time <= third_method_time
        ):
            print("it is better to use a loop")
        elif (
            second_method_time <= first_method_time
            and second_method_time <= third_method_time
        ):
            print("it is better to use a list comprehension")
        else:
            print("it is better to use a map")

        times = sorted([first_method_time, second_method_time, third_method_time])
        print(f"{times[0]:.6f} vs {times[1]:.6f} vs {times[2]:.6f}")
